...
DROP TRIGGER IF EXISTS normalize_lunch_fields_trigger ON public.timesheets;
DROP TRIGGER IF EXISTS set_timesheet_end_date_trigger ON public.timesheets;
DROP TRIGGER IF EXISTS calculate_timesheet_hours_trigger ON public.timesheets;
DROP TRIGGER IF EXISTS recalculate_on_timesheet_change_trigger ON public.timesheets;
DROP TRIGGER IF EXISTS calculate_timesheet_hours_from_sessions_trigger ON public.timesheets;

-- Drop session triggers
DROP TRIGGER IF EXISTS update_timesheet_sessions_updated_at ON public.timesheet_sessions;
DROP TRIGGER IF EXISTS calculate_hours_on_session_change_trigger ON public.timesheet_sessions;

-- Create the correct trigger function for calculating hours from sessions
...
DECLARE
    ...
    first_session_start timestamp with time zone;
BEGIN
    ...
    LOOP
        ...
        -- Calculate night hours for this session (same logic as before)
    ...
    -- Get first session for day calculation
    SELECT start_time INTO first_session_start 
    ...
    IF first_session_start IS NOT NULL THEN
        local_start_time := first_session_start AT TIME ZONE 'Europe/Rome';
    ...
    -- Update the timesheet record with calculated values
    UPDATE public.timesheets 
    SET 
        ...
        is_saturday = COALESCE(is_saturday, false),
        updated_at = now()
    WHERE id = timesheet_record.id;
...
-- Create triggers for session changes
CREATE TRIGGER calculate_hours_on_session_change_trigger
...
-- Re-create essential timesheet triggers (but NOT the hours calculation one)
CREATE TRIGGER update_timesheets_updated_at
    BEFORE UPDATE ON public.timesheets
    FOR EACH ROW
    EXECUTE FUNCTION public.updated_at();

CREATE TRIGGER normalize_lunch_fields_trigger
    BEFORE INSERT OR UPDATE ON public.timesheets
    FOR EACH ROW
    EXECUTE FUNCTION public.normalize_lunch_fields();
