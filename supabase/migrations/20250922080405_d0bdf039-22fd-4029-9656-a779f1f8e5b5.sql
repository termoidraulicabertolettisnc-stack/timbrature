-- First check the conflicting data
...
BEGIN
    -- Get user IDs
    SELECT user_id INTO mihai_uuid FROM profiles WHERE first_name = 'Mihai' LIMIT 1;
    SELECT user_id INTO lorenzo_uuid FROM profiles WHERE first_name = 'Lorenzo' LIMIT 1;
    
    -- Count conflicting records
    SELECT COUNT(*) INTO conflict_count
    FROM timesheets
    WHERE (lunch_duration_minutes IS NOT NULL)
      AND (lunch_start_time IS NOT NULL OR lunch_end_time IS NOT NULL)
      AND date BETWEEN '2025-08-01' AND '2025-08-31';
    
    RAISE NOTICE 'Found % conflicting lunch records in August 2025', conflict_count;
    
    -- Show some examples
    RAISE NOTICE 'Examples of conflicting records:';
    FOR record IN 
        SELECT id, date, user_id, lunch_duration_minutes, lunch_start_time, lunch_end_time
        FROM timesheets
        WHERE (lunch_duration_minutes IS NOT NULL)
          AND (lunch_start_time IS NOT NULL OR lunch_end_time IS NOT NULL)
          AND date BETWEEN '2025-08-01' AND '2025-08-31'
        LIMIT 5
    LOOP
        RAISE NOTICE 'ID: %, Date: %, Duration: %, Start: %, End: %', 
            record.id, record.date, record.lunch_duration_minutes, 
            record.lunch_start_time, record.lunch_end_time;
    END LOOP;
END $$;

-- 1. Create normalization trigger to prevent future conflicts
...
-- Create trigger with high priority (execute before calculation trigger)
CREATE TRIGGER trg_normalize_lunch_fields
BEFORE INSERT OR UPDATE ON public.timesheets
FOR EACH ROW EXECUTE FUNCTION public.normalize_lunch_fields();

-- 2. Clean up existing conflicting data (prioritize duration over times)
UPDATE public.timesheets
SET lunch_start_time = NULL, lunch_end_time = NULL
WHERE lunch_duration_minutes IS NOT NULL
  AND (lunch_start_time IS NOT NULL OR lunch_end_time IS NOT NULL)
  AND date BETWEEN '2025-08-01' AND '2025-08-31'
  AND user_id IN (
    SELECT user_id FROM public.profiles
    WHERE first_name IN ('Mihai','Lorenzo')
  );

-- 3. Fix the calculation trigger to properly find employee settings
...
BEGIN
    ...
    -- FIXED: Get employee settings with proper date-based lookup
    ...
    -- FIXED: Determine lunch break settings with proper priority
...
-- 4. Force recalculation for Mihai and Lorenzo's August data
UPDATE public.timesheets
SET updated_at = now()
...
SELECT 'AFTER CLEANUP - Remaining conflicts:' as status, COUNT(*) as count
FROM timesheets
WHERE (lunch_duration_minutes IS NOT NULL)
  AND (lunch_start_time IS NOT NULL OR lunch_end_time IS NOT NULL)
  AND date BETWEEN '2025-08-01' AND '2025-08-31';