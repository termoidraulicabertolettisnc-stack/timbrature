-- Update the calculate_timesheet_hours function to use configurable meal_voucher_min_hours
...
DECLARE
    ...
    effective_meal_allowance_policy text;
    effective_meal_voucher_min_hours integer;
...
BEGIN
    ...
    -- Ottieni le impostazioni del dipendente specifico (ordinate per updated_at DESC per gestire eventuali duplicati)
    ...
    -- Ottieni le ore standard per questo giorno specifico
    standard_daily_hours_for_day := COALESCE((weekly_hours_config->>day_name)::integer, 8);

    -- Determina la policy unificata per buoni pasto/indennità e le ore minime
    IF employee_settings_rec IS NOT NULL AND employee_settings_rec.meal_allowance_policy IS NOT NULL THEN
        effective_meal_allowance_policy := employee_settings_rec.meal_allowance_policy::text;
        effective_meal_voucher_min_hours := COALESCE(employee_settings_rec.meal_voucher_min_hours, 6);
    ELSIF company_settings_rec IS NOT NULL AND company_settings_rec.meal_allowance_policy IS NOT NULL THEN
        effective_meal_allowance_policy := company_settings_rec.meal_allowance_policy::text;
        effective_meal_voucher_min_hours := COALESCE(company_settings_rec.meal_voucher_min_hours, 6);
    ELSE
        effective_meal_allowance_policy := 'disabled';
        effective_meal_voucher_min_hours := 6;
    END IF;
    ...
    -- Calcola ore straordinarie correttamente per il sabato
    IF is_saturday THEN
        IF saturday_handling_type = 'trasferta' THEN
            -- Se il sabato è gestito come trasferta, tutte le ore sono ordinarie, nessuna straordinaria
            calculated_overtime_hours := 0;
        ELSE
            -- Se il sabato è gestito come straordinario, le ore oltre quelle standard sono straordinarie
            -- Se standard_daily_hours_for_day è 0 per sabato, allora tutte le ore sono straordinarie
        ...
        END IF;
    ELSE
        -- Per gli altri giorni, calcola normalmente
    ...
    -- Determina buono pasto basato sulla nuova policy unificata e ore minime configurabili
    IF effective_meal_allowance_policy = 'disabled' OR effective_meal_allowance_policy = 'daily_allowance' THEN
        NEW.meal_voucher_earned := false;
    ELSIF effective_meal_allowance_policy = 'meal_vouchers_only' THEN
        NEW.meal_voucher_earned := NEW.total_hours >= effective_meal_voucher_min_hours;
    ELSIF effective_meal_allowance_policy = 'meal_vouchers_always' THEN
