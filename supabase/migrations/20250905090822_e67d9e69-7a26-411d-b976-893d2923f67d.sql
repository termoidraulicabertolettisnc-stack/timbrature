-- Fix the calculate_timesheet_hours function to use correct lunch break duration
-- based on employee and company settings
...
BEGIN
    ...
    ELSE
        -- Se non specificata, sottrai la pausa pranzo configurata se ha lavorato più di 6 ore
...
-- Update existing timesheets to recalculate hours with the new logic
-- This will trigger the updated function for all existing records
UPDATE public.timesheets 
SET updated_at = now() 
WHERE start_time IS NOT NULL 
  AND end_time IS NOT NULL 
  AND (lunch_start_time IS NULL OR lunch_end_time IS NULL);