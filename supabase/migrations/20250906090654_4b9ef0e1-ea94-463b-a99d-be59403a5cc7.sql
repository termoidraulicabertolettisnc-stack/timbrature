-- Add absence_type enum if not exists
DO $$ BEGIN
    CREATE TYPE absence_type AS ENUM ('A', 'F', 'FS', 'I', 'M', 'PR', 'PNR');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

-- Add absence fields to timesheets table (only if they don't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'timesheets' AND column_name = 'absence_type') THEN
        ALTER TABLE public.timesheets ADD COLUMN absence_type absence_type;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'timesheets' AND column_name = 'is_absence') THEN
        ALTER TABLE public.timesheets ADD COLUMN is_absence boolean DEFAULT false;
    END IF;
END $$;

-- Insert the three companies (only if they don't exist)
INSERT INTO public.companies (id, name, city, address) VALUES 
('11111111-1111-1111-1111-111111111111', 'Termoidraulica', 'Cremona', NULL),
('22222222-2222-2222-2222-222222222222', 'Piramide', 'Cremona', NULL),
('33333333-3333-3333-3333-333333333333', 'Meridiana', 'Cremona', NULL)
ON CONFLICT (id) DO NOTHING;

-- Update the timesheet calculation function to handle weekly hours and absences
...
BEGIN
    ...
    -- Resto della logica come prima, ma con altre impostazioni
    ...
    -- Calcola ore straordinarie usando le ore standard del giorno specifico
