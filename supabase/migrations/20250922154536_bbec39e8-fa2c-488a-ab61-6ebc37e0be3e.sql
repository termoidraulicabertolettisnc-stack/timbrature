-- Fix the trigger issue and migrate existing data

-- First, drop the existing session trigger that doesn't work correctly  
DROP TRIGGER IF EXISTS recalculate_timesheet_hours_on_session_change ON public.timesheet_sessions;

-- Create a fixed session-based calculation trigger
