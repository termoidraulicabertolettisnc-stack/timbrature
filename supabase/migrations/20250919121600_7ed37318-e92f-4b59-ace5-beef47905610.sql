-- Fix the night hours calculation in calculate_timesheet_hours trigger
...
DECLARE
    ...
    night_start_current timestamp with time zone;
    night_end_next timestamp with time zone;
    night_start_previous timestamp with time zone;
    night_end_current timestamp with time zone;
    night_overlap_minutes numeric := 0;
    temp_start timestamp with time zone;
    temp_end timestamp with time zone;
BEGIN
    ...
    IF night_start_time > night_end_time THEN
        -- Definisci i periodi notturni per il giorno del turno e il giorno successivo
        night_start_current := DATE(work_start_time) + night_start_time::time;
        night_end_next := DATE(work_start_time) + INTERVAL '1 day' + night_end_time::time;
        
        -- Anche il giorno precedente potrebbe essere rilevante
        night_start_previous := DATE(work_start_time) - INTERVAL '1 day' + night_start_time::time;
        night_end_current := DATE(work_start_time) + night_end_time::time;
        
        -- Calcola sovrapposizione con la parte notturna del giorno precedente (se il turno inizia prima delle 05:00)
        IF work_start_time < night_end_current AND work_end_time > (DATE(work_start_time) + '00:00:00'::time) THEN
            temp_start := GREATEST(work_start_time, DATE(work_start_time) + '00:00:00'::time);
            temp_end := LEAST(work_end_time, night_end_current);
        ...
        IF work_start_time < (DATE(work_start_time) + INTERVAL '1 day') AND work_end_time > night_start_current THEN
            temp_start := GREATEST(work_start_time, night_start_current);
            temp_end := LEAST(work_end_time, DATE(work_start_time) + INTERVAL '1 day');
            IF temp_end > temp_start THEN
                night_overlap_minutes := night_overlap_minutes + EXTRACT(EPOCH FROM (temp_end - temp_start)) / 60;
            END IF;
        END IF;
        ...
        IF work_start_time < night_end_next AND work_end_time > (DATE(work_start_time) + INTERVAL '1 day') THEN
            temp_start := GREATEST(work_start_time, DATE(work_start_time) + INTERVAL '1 day');
            temp_end := LEAST(work_end_time, night_end_next);
            IF temp_end > temp_start THEN
                night_overlap_minutes := night_overlap_minutes + EXTRACT(EPOCH FROM (temp_end - temp_start)) / 60;
            END IF;
        END IF;
    ELSE
        ...
        IF work_start_time < night_end_current AND work_end_time > night_start_current THEN
            temp_start := GREATEST(work_start_time, night_start_current);
            temp_end := LEAST(work_end_time, night_end_current);
