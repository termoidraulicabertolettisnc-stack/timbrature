-- Fix the calculate_timesheet_hours trigger to properly handle lunch break priority
-- Priority should be: timesheet specific > employee settings > company settings > default
...
BEGIN
    ...
    -- Get employee settings for the specific date with better query
    ...
    -- FIXED: Improved lunch break determination with correct NULL handling and priority
    -- Priority: timesheet specific times > timesheet duration > employee settings > company settings > default
    IF NEW.lunch_start_time IS NOT NULL AND NEW.lunch_end_time IS NOT NULL THEN
        -- Timesheet has specific lunch times - use those (highest priority)
        lunch_break_minutes := EXTRACT(EPOCH FROM (NEW.lunch_end_time - NEW.lunch_start_time)) / 60;
    ELSIF NEW.lunch_duration_minutes IS NOT NULL THEN
        -- Timesheet has specific duration - use that (second priority)
        lunch_break_minutes := NEW.lunch_duration_minutes;
    ELSE
        -- No timesheet-specific lunch data, check employee settings then company settings
        IF employee_settings_rec IS NOT NULL AND employee_settings_rec.lunch_break_type IS NOT NULL THEN
            -- Use employee settings (third priority)
        ...
        ELSIF company_settings_rec IS NOT NULL AND company_settings_rec.lunch_break_type IS NOT NULL THEN
            -- Use company settings (fourth priority)
    ...
    -- Continue with other settings determination
    ...
    -- Night hours calculation (keeping existing logic)
...
-- Also fix the debug function to match the new logic
...
-- Force recalculation of Mihai's timesheets by updating them
UPDATE public.timesheets 
SET updated_at = now() 
WHERE user_id = '98e32095-a0dd-4453-ab7e-ed30cd069583'  -- Mihai's user_id
AND lunch_duration_minutes IS NULL 
AND lunch_start_time IS NULL 
