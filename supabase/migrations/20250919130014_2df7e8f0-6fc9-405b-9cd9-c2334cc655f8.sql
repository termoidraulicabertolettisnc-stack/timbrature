-- Fix timezone conversion in calculate_timesheet_hours function
...
BEGIN
    ...
    -- FIXED: Conversione corretta alla timezone Europe/Rome (gestisce automaticamente ora legale/solare)
...
-- Riattiva il trigger per tutti i timesheet
...
-- Ricalcola tutti i timesheet di agosto 2025 per Mihai per correggere i night hours
UPDATE timesheets 
SET start_time = start_time
WHERE user_id IN (
    SELECT user_id FROM profiles WHERE first_name = 'Mihai' AND last_name = 'Federciuc'
) 
AND date >= '2025-08-01' 
AND date <= '2025-08-31';