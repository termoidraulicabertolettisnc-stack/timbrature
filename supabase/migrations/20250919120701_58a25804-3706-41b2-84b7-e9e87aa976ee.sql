-- Correzione logica calcolo ore notturne nel trigger
-- Il trigger attuale assegna tutte le ore come notturne se anche solo l'inizio o la fine
-- del turno cade nel periodo notturno. Invece deve calcolare solo le ore effettive
-- che cadono nel periodo notturno (22:00-05:00)
...
BEGIN
    ...
    IF NEW.is_absence = true THEN
        ...
        NEW.night_hours := 0;
        -- Keep meal_voucher_earned as is for absences (usually false)
        RETURN NEW;
    END IF;
    ...
    -- FIXED: Ottieni le impostazioni del dipendente valide per questa data specifica
    ...
    -- Calcola ore straordinarie SEMPRE dopo le ore standard giornaliere
    IF is_saturday THEN
        ...
        END IF;
    ELSE
        -- Logica semplificata: straordinari sempre dopo le ore standard giornaliere
    ...
    -- FIXED: Calcola ore notturne correttamente
    -- Calcola solo le ore effettive che cadono nel periodo notturno
    work_start_time := NEW.start_time;
    work_end_time := NEW.end_time;
    
    -- Definisci il periodo notturno per oggi e domani
    night_start_today := DATE(NEW.date) + night_start_time::time;
    night_end_tomorrow := DATE(NEW.date) + INTERVAL '1 day' + night_end_time::time;
    
    -- Se il periodo notturno attraversa la mezzanotte (es. 22:00-05:00)
    IF night_start_time > night_end_time THEN
        -- Calcola sovrapposizione con la parte di oggi (dopo night_start)
        IF work_start_time <= night_start_today + INTERVAL '1 day' AND work_end_time >= night_start_today THEN
            night_overlap_minutes := night_overlap_minutes + EXTRACT(EPOCH FROM 
                LEAST(work_end_time, night_start_today + INTERVAL '1 day') - 
                GREATEST(work_start_time, night_start_today)
            ) / 60;
        END IF;
        
        -- Calcola sovrapposizione con la parte di domani (prima di night_end)
        IF work_start_time <= night_end_tomorrow AND work_end_time >= DATE(NEW.date) + INTERVAL '1 day' THEN
            night_overlap_minutes := night_overlap_minutes + EXTRACT(EPOCH FROM 
                LEAST(work_end_time, night_end_tomorrow) - 
                GREATEST(work_start_time, DATE(NEW.date) + INTERVAL '1 day')
            ) / 60;
        END IF;
    ELSE
        -- Il periodo notturno è nello stesso giorno
        IF work_start_time <= DATE(NEW.date) + night_end_time::time AND work_end_time >= night_start_today THEN
            night_overlap_minutes := EXTRACT(EPOCH FROM 
                LEAST(work_end_time, DATE(NEW.date) + night_end_time::time) - 
                GREATEST(work_start_time, night_start_today)
            ) / 60;
        END IF;
    END IF;
    ...
    -- NOTE: meal_voucher_earned is no longer calculated here
    -- The frontend will handle all meal benefit calculations
    -- We keep the field in the database for reporting purposes only
