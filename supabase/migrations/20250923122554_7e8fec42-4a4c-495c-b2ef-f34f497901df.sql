-- Drop existing trigger that calculates from start_time/end_time
DROP TRIGGER IF EXISTS timesheet_hours_trigger ON public.timesheets;

-- Create trigger that calculates hours from sessions whenever sessions change
CREATE OR REPLACE FUNCTION calculate_hours_on_session_change() 
RETURNS TRIGGER AS $$
DECLARE
...
CREATE TRIGGER session_hours_trigger
    AFTER INSERT OR UPDATE OR DELETE ON public.timesheet_sessions
    FOR EACH ROW EXECUTE FUNCTION calculate_hours_on_session_change();

-- Also ensure we recalculate when timesheets are inserted/updated (for existing logic compatibility)
CREATE OR REPLACE FUNCTION recalculate_on_timesheet_change()
RETURNS TRIGGER AS $$
BEGIN
    -- For existing timesheets that might not have sessions yet, use old calculation
    -- But if sessions exist, they will trigger the session-based calculation
    IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
        -- Check if this timesheet has sessions
        IF EXISTS (
            SELECT 1 FROM public.timesheet_sessions 
            WHERE timesheet_id = NEW.id AND session_type = 'work'
        ) THEN
            -- Sessions exist, let the session trigger handle it
            RETURN NEW;
        ELSE
            -- No sessions, use old calculation method for backwards compatibility
            RETURN NEW;
        END IF;
    END IF;
...
CREATE TRIGGER timesheet_recalc_trigger
    AFTER INSERT OR UPDATE ON public.timesheets
    FOR EACH ROW EXECUTE FUNCTION recalculate_on_timesheet_change();