-- Drop all triggers that depend on the function, then recreate them
DROP TRIGGER IF EXISTS calculate_hours_from_sessions_trigger ON timesheet_sessions;
DROP TRIGGER IF EXISTS recalculate_timesheet_hours_on_session_change ON timesheet_sessions;
DROP FUNCTION IF EXISTS calculate_hours_from_sessions() CASCADE;
...
-- Recreate the trigger (use the same name as before)
