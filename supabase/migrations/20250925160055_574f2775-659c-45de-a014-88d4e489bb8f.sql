-- CORREZIONE BUG: calculate_timesheet_hours_legacy non applica correttamente le ore minime per pausa pranzo
-- Il problema è che la funzione calcola ore sbagliate quando non dovrebbe applicare la pausa
...
BEGIN
    ...
    -- Get lunch break minimum hours - CORREZIONE: Usa il campo corretto
    ...
    -- CORREZIONE PRINCIPALE: Calcolo della pausa pranzo con logica corretta
    ...
    ELSE
        ...
        -- CORREZIONE: Applica la pausa solo se le ore lavorate sono MAGGIORI delle ore minime
    ...
    -- CORREZIONE: Assicurati che non vada sotto zero
...
-- Forza il ricalcolo dei timesheet problematici
UPDATE timesheets 
SET updated_at = NOW() 
WHERE date >= '2025-09-20' 
  AND total_hours != ROUND(EXTRACT(EPOCH FROM (end_time - start_time)) / 3600.0, 2)
