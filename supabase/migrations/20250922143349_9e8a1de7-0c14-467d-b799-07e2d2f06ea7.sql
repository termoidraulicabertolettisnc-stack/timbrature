-- Fix calculate_timesheet_hours function to properly handle multiple work sessions
-- The function should NOT recalculate total_hours if it's already set from import
...
DECLARE
    ...
    -- Variables for employee settings lookup fix
...
BEGIN
    ...
    -- CRITICAL FIX: If total_hours is already set (from import), don't recalculate it
    -- This preserves the correct calculation from ExcelImportService for multiple sessions
    IF NEW.total_hours IS NOT NULL AND NEW.total_hours > 0 THEN
        -- Skip total_hours calculation but still calculate overtime and night hours
        NULL;
    ELSE
        -- Original calculation logic for manual entries
    ...
    -- Always calculate overtime and night hours based on local time
    local_start_time := NEW.start_time AT TIME ZONE 'Europe/Rome';
    local_end_time := NEW.end_time AT TIME ZONE 'Europe/Rome';
