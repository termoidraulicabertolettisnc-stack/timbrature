-- Debug function to trace lunch break calculation
...
-- Fixed calculate_timesheet_hours function with correct lunch break logic
...
BEGIN
    ...
    -- FIXED: Get employee settings for the specific date with better query
    SELECT es.* INTO employee_settings_rec
    FROM public.employee_settings es
    ...
    -- FIXED: Improved lunch break determination with explicit NULL checks
    -- Priority: timesheet specific > employee settings > company settings > default
    ...
    ELSIF NEW.lunch_duration_minutes IS NOT NULL AND NEW.lunch_duration_minutes > 0 THEN
        -- Timesheet has specific duration - use that (highest priority)
        lunch_break_minutes := NEW.lunch_duration_minutes;
    ELSIF employee_settings_rec IS NOT NULL AND employee_settings_rec.lunch_break_type IS NOT NULL THEN
        -- Use employee settings (second priority)
    ...
    ELSIF company_settings_rec IS NOT NULL AND company_settings_rec.lunch_break_type IS NOT NULL THEN
        -- Use company settings (third priority)
    ...
    -- FIXED: Apply lunch break logic correctly
    IF NEW.lunch_start_time IS NOT NULL AND NEW.lunch_end_time IS NOT NULL THEN
    ...
    ELSE
        -- Apply automatic lunch break if shift is long enough and lunch break is configured
