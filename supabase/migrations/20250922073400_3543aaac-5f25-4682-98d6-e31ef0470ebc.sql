-- Fix lunch break calculation trigger with proper overlap logic and local time handling
...
BEGIN
    ...
    -- Determine lunch break settings (same logic as before but will be applied differently)
    ...
    IF NEW.lunch_start_time IS NOT NULL AND NEW.lunch_end_time IS NOT NULL THEN
        ...
        IF lunch_end_tz > shift_start AND lunch_start_tz < shift_end THEN
            lunch_overlap_ms := EXTRACT(EPOCH FROM (
                LEAST(shift_end, lunch_end_tz) - GREATEST(shift_start, lunch_start_tz)
            )) * 1000;
            lunch_overlap_ms := GREATEST(0, lunch_overlap_ms);
        END IF;
        
    ELSIF NEW.lunch_duration_minutes IS NOT NULL THEN
        -- Caso 2: Durata specifica - cap alla durata del turno
        lunch_overlap_ms := LEAST(
            EXTRACT(EPOCH FROM work_duration) * 1000,
            (NEW.lunch_duration_minutes::numeric * 60 * 1000)
        );
    ...
    ELSE
        ...
        IF lunch_break_minutes > 0 AND hours_worked_without_lunch > 6 THEN
            lunch_overlap_ms := LEAST(
                EXTRACT(EPOCH FROM work_duration) * 1000,
                (lunch_break_minutes::numeric * 60 * 1000)
            );
        END IF;
    END IF;

    -- Applica la pausa calcolata
    work_duration := work_duration - MAKE_INTERVAL(msecs := lunch_overlap_ms);
...
-- F) SOLUZIONE: Force recalculation for affected timesheets
-- Update all timesheets for users with lunch break issues to trigger recalculation
UPDATE public.timesheets
SET updated_at = now()
WHERE date >= '2025-08-01' 
  AND date <= '2025-08-31'
  AND (lunch_duration_minutes IS NULL OR lunch_duration_minutes != 30)
  AND user_id IN (
    SELECT DISTINCT t.user_id 
    FROM timesheets t
    JOIN profiles p ON p.user_id = t.user_id
    WHERE p.first_name IN ('Mihai', 'Lorenzo') -- Target affected users
  );