-- Step 2: Aggiorna i record esistenti per usare il nuovo valore 'both' al posto di 'meal_vouchers_always'
...
-- Aggiorna la funzione di calcolo per gestire l'opzione 'both'
...
BEGIN
    ...
    -- Determina buono pasto basato sulla nuova policy unificata
    ...
    ELSIF effective_meal_allowance_policy = 'both' THEN
        -- Con la policy 'both', viene data preferenza ai buoni pasto se si raggiungono le ore minime
        NEW.meal_voucher_earned := NEW.total_hours >= effective_meal_voucher_min_hours;
    ELSE
        NEW.meal_voucher_earned := false;
    END IF;
