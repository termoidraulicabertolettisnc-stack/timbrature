...
const processTimesheetSessions = (timesheet: TimesheetWithProfile): ExtendedTimesheetWithProfile[] => {
  ...
  console.log(
    "🔍 SESSIONI MULTIPLE - Processing timesheet:",
    timesheet.id,
    "with sessions:",
  ...
  if (timesheet.timesheet_sessions && timesheet.timesheet_sessions.length > 0) {
    ...
    timesheet.timesheet_sessions.forEach((session, index) => {
      if (session.start_time) {
        ...
        console.log(
          "🔍 SESSIONI MULTIPLE - Added session:",
          sessionTimesheet.id,
          "from",
          session.start_time,
          "to",
  ...
  if (!session.end_time && session.start_time) {
    return (
      <span className="text-blue-600">
        ...
        {rtNightHours > 0 && <span className="text-xs text-blue-700 ml-1">• notturne {rtNightHours.toFixed(1)}h</span>}
        {session.is_session && (
          <span className="text-xs text-purple-700 ml-1">• sessione #{session.session_order || 1}</span>
        )}
      </span>
    );
  }
  ...
  return (
    <span>
      ...
      {session.night_hours && session.night_hours > 0 && (
        <span className="text-xs text-muted-foreground ml-1">• notturne {session.night_hours.toFixed(1)}h</span>
      )}
      {session.is_session && (
      ...
      <div className="text-xs text-gray-600">
        ...
        {employee.overtime_hours > 0 && (
          <>
            ...
            <span className="text-orange-600 font-medium">{formatHours(employee.overtime_hours)}</span>
          </>
        )}
...
export default function AdminTimesheets() {
  ...
  const loadSettings = async () => {
    try {
      ...
      const { data: employeeData, error: employeeError } = await supabase.from("employee_settings").select("*");
  ...
  const getMealBenefits = (timesheet: TimesheetWithProfile) => {
    ...
    return BenefitsService.calculateMealBenefitsSync(timesheet, employeeSettingsForUser, companySettings);
  };
  ...
  const handleDeleteTimesheetUnified = async (
    ...
    try {
      ...
      if (deleteType === "session" && sessionId) {
        ...
        const { error: sessionsError } = await supabase.from("timesheet_sessions").delete().eq("timesheet_id", realId);
        ...
        const { error: timesheetError } = await supabase.from("timesheets").delete().eq("id", realId);
      ...
      toast({
        title: "Successo",
        description: deleteType === "session" ? "Sessione eliminata con successo" : "Timesheet eliminato con successo",
      });
  ...
  const WeeklyTimelineEntryActions = ({ entry, onDeleteTimesheet, onEditTimesheet, originalId }: any) => {
    const handleDeleteClick = (e: React.MouseEvent) => {
      ...
      if (isMultiSession && sessionId) {
        ...
        handleDeleteTimesheetUnified(originalId, "session", sessionId).then(
          (success) => success && onDeleteTimesheet?.(originalId),
        );
      } else {
        // Elimina tutto il timesheet
        handleDeleteTimesheetUnified(originalId, "timesheet").then(
          (success) => success && onDeleteTimesheet?.(originalId),
        );
      }
    };
  ...
  const MonthlyDayActions = ({
    ...
    const handleDeleteDay = () => {
      ...
      handleDeleteTimesheetUnified(mainTimesheet.id, "timesheet").then(
        (success) => success && onDeleteTimesheet(mainTimesheet.id),
      );
    };
  ...
  const DailySummarySessionActions = ({ session, onDeleteTimesheetLocal, onEditTimesheet }: any) => {
    const handleDeleteSession = () => {
      ...
      if (isSpecificSession && sessionId) {
        ...
        handleDeleteTimesheetUnified(originalId, "session", sessionId).then(
          (success) => success && onDeleteTimesheetLocal?.(originalId),
        );
      } else {
        // Elimina tutto il timesheet
        handleDeleteTimesheetUnified(originalId, "timesheet").then(
    ...
    return (
      <div className="flex gap-1">
        <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => onEditTimesheet(session)}>
  ...
  const loadAbsences = async (startDate: Date, endDate: Date) => {
    try {
      console.log(
        "🔍 Caricamento assenze per periodo:",
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Visualizza e gestisci i timesheet di tutti i dipendenti</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="secondary" onClick={() => setMassTimesheetDialogOpen(true)} className="gap-2">
      ...
      {/* Dialog per inserimento presenze multiple */}
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Visualizzazione aggregata per dipendente con tutte le sessioni multiple</CardDescription>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" onClick={onNavigatePrevious} title="Giorno precedente">
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <Button variant="outline" onClick={onNavigateToday} className="min-w-[80px]">
              Oggi
            </Button>
            <Button variant="outline" size="icon" onClick={onNavigateNext} title="Giorno successivo">
